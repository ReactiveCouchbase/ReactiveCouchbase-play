...
object PlayCouchbase {
  ...
  private[reactivecouchbase] val maybeCouchbaseActorSystem: Option[ActorSystem] = None
  ...
  def cappedBucket(b: String, max: Int, reaper: Boolean = true)(implicit app: Application): CappedBucket = {
    ...
    //buckets(app).get(b).map { _ =>
    //  CappedBucket(() => bucket(b), bucket(b).driver.executor(), max, reaper)
    //}.getOrElse(throw new PlayException(s"Error with bucket $b", s"Bucket '$b' is not defined"))
  }
  ...
  def couchbaseActorSystem(implicit app: Application) = {
    maybeCouchbaseActorSystem.getOrElse(ActorSystem("reactivecouchbase-plugin-system", Play.configuration(app).underlying.getConfig("couchbase.actorctx")))
  }
}
